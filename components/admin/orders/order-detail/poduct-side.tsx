...
const ProductSide = ({ order }: { order?: Order }) => {
    return (
        <Card sx={{ backgroundColor: "#27293d", flex: 1, py: 2, px: 3 }}>
            ...
            <Box mt={2}>
                <Typography variant="body2" sx={{ color: "#c3c3c3" }} mt={1}>
                    สั่งซื้อเมื่อ: <a className="text-white">{format(order?.createdAt || "", "d MMM yyyy h:mm a")}</a>
                </Typography>
                <Typography variant="body2" sx={{ color: "#c3c3c3" }} mt={1}>สถานะ: <a className="text-white">{order?.status === "PENDING" ? "รอดำเนินการ" : (order?.status === "COMPLETED" ? "สำเร็จ" : "ยกเลิก")}</a></Typography>
                <Typography variant="body2" sx={{ color: "#c3c3c3" }} mt={1}>
            ...
            <CardContent>
                    ...
                    {order?.items.map((item, index) => (
                        <Stack key={index} direction="row" spacing={2} justifyContent={"space-between"}>
                            <Box display={'flex'} justifyContent={'center'} alignItems={"center"}>
                                ...
                                <Box ml={2}>
                                    <Typography variant="body1" fontWeight={600} sx={{ color: "#ffff" }}>{item.product.title}</Typography>
                ...
                <Box mt={3} display={'flex'} justifyContent={'end'} alignItems={'center'}>
